...
public class ThongKeNhanVien_DAO {
    ...
    public KetQuaThongKe getThongKe(java.util.Date tuNgay, java.util.Date denNgay, String maNhanVien, int caLam) {
        ...
        try {
            ...
            if (maNhanVien != null)
                psBan.setString(index++, maNhanVien); // Chỉ set nếu khác null
            if (caLam > 0)
                psBan.setInt(index++, caLam);
            ...
            if (maNhanVien != null)
                psTra.setString(index++, maNhanVien);
            if (caLam > 0)
                psTra.setInt(index++, caLam);
            ...
            if (maNhanVien != null)
                psHuy.setString(index++, maNhanVien);
            if (caLam > 0)
                psHuy.setInt(index++, caLam);
    ...
    public List<ThongKeChiTietNV> getThongKeDanhSachNhanVien(java.util.Date tuNgay, java.util.Date denNgay) {
        ...
        // Kỹ thuật: Sử dụng LEFT JOIN với các bảng con (Derived Tables) để gom nhóm dữ
        // liệu
        String sql = """
                    SELECT
                        nv.MaNhanVien,
