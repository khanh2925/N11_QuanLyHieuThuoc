...
public class ThemNhaCungCap_Dialog extends JDialog implements ActionListener {
	...
	private void initUI() {
		...
		txtSdt.setBounds(12, 195, 470, 68);
		...
		txtDiaChi.setSize(470, 58);
		txtDiaChi.setLocation(12, 195);
		txtDiaChi.setBounds(12, 110, 470, 36);
	...
	private void themNCC() {
		...
		// SĐT Việt Nam phổ biến: bắt đầu bằng 0, tổng 10 hoặc 11 số
		if (!sdt.matches("^0\\d{9,10}$")) {
			showWarn("Số điện thoại không hợp lệ. Vui lòng nhập 10–11 số và bắt đầu bằng 0.");
			txtSdt.requestFocus();
			return;
		}
		...
		// Sinh mã NCC dạng NCC-xxx (hoặc theo DB của bạn)
		...
		nhaCungCapMoi = ncc;
		JOptionPane.showMessageDialog(this, "Đã thêm nhà cung cấp mới: " + ten, "Thành công",
